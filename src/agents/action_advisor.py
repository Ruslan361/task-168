            ...
            action = call_mws_gpt([
                {
                    ...
                    "content": f"""
                        ...
                        Если подтверждена проблема (например, сбой приложения, списание средств), добавь действие: 'Предложить клиенту скидку 20% на следующий месяц услуг.'
                        Если запрошен недоступный сервис (например, карпулинг), добавь действие: 'Проинформировать клиента об альтернативных сервисах МТС, таких как {alternative_service or 'МТС Такси'}' # Provide default if empty
